...
function Invoke-CSharpDotnet {
    ...
    dotnet run --project _external\csharp-lsp\CSharpLspSmokeTest\CSharpLspSmokeTest.csproj -- @args
}
...
function Invoke-CSharpNode {
    ...
    node -r "$env:NODE_PATH\ts-node\register\transpile-only" _external\csharp-lsp\node-client.ts @args
}
