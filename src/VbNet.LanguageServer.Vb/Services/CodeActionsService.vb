...
Namespace Services
  ...
  Public NotInheritable Class CodeActionsService
    ...
    Private Shared Function TryGetTextSpan([range] As Protocol.Range, sourceText As SourceText) As TextSpan?
      ...
      Dim startPosition = CInt(Math.Min(CLng(startLine.Start) + CLng([range].Start.Character), CLng(startLine.End)))
      Dim endPosition = CInt(Math.Min(CLng(endLine.Start) + CLng([range].End.Character), CLng(endLine.End)))
